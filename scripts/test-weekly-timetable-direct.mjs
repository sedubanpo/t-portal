...
assert.match(indexText, /teacher_name=eq\.' \+ encodeURIComponent\(teacherName\)/);
...
assert.match(indexText, /document\.body\.dataset\.teacherTimetableLoadSource = 'supabase-direct'/);
...
assert.match(loaderBlock, /fetchWeekTimetableMonthDirect\(Object.assign/);
assert.doesNotMatch(loaderBlock, /fetchWeekTimetableMonthLegacy/);

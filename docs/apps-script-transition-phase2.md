...
활성 강사 31명의 시수 조회 canary가 준비되어 있다. 과거 월과 현재 월 모두 Supabase 요약을 직접 읽는다. 현재 월은 5분 이내의 최신 요약만 허용하고, 요약 누락·stale·요청 실패 시 기존 Apps Script로 자동 복귀한다. 안준성은 관리자 범위, 일반 강사는 본인 강사 범위만 읽을 수 있다.
...
window.__TPORTAL_SUPABASE_PUBLIC_CONFIG__ = {
  ...
  // 실제 목록은 canaryFirebaseUids와 같은 활성 강사 31명이다.
  currentMonthDirectFirebaseUids: ['teacher_...'],
...
11. 활성 강사 31명의 현재 월 동등성·최신성·교차 범위 차단 확인 및 직접 읽기 확대 — 완료
...
현재 월은 활성 강사 31명이 5분 이내의 최신 Supabase 요약을 직접 읽는다. 2026년 7월 검사에서 31/31 GAS/Supabase 결과 동등성과 최신성을 통과했고, 일반 강사 30/30의 타 강사 범위 요청도 차단됐다. 강제 새로고침과 모든 쓰기 action은 Apps Script 경로를 유지한다. Access 업로드·복구·Firebase 동기화가 발생하면 영향 월의 Supabase 요약을 삭제하며, 이후 조회는 GAS fallback으로 최신 요약을 재생성한 뒤 직접 읽기를 재개한다. 운영 출결·시수 원본 자체는 이 전환에서 수정하지 않는다.

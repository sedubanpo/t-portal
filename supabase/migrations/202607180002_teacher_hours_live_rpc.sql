-- Canonical live teacher-hours read path.
-- It keeps Access daily-batch replacement semantics and class-log agreement
-- fallback on the server without exposing import batch metadata to teachers.
...
begin
  ...
  if not private.portal_can_access_teacher(
    private.portal_teacher_id_by_name(v_teacher),
    v_teacher
  ) then
  ...
  with teacher_dates as (
    ...
    where a.class_date >= v_start
      and a.class_date < v_end
      and a.teacher_name = v_teacher
  ), latest as (
    select d.class_date,
      batch.id,
      batch.expected_rows,
      coalesce((
        select count(*)
        from public.attendance_logs a
        where a.class_date = d.class_date and a.import_batch_id = batch.id
      ), 0)::integer as actual_rows
    from teacher_dates d
    left join lateral (
      select b.id,
        case
          when snapshot.snapshot_rows > 0 then snapshot.snapshot_rows
          when jsonb_typeof(b.metadata -> 'sourceDates') = 'array'
               and jsonb_array_length(b.metadata -> 'sourceDates') = 1
            then coalesce(b.row_count, 0)
      ...
      cross join lateral (
        ...
        from jsonb_array_elements(
          case when jsonb_typeof(b.metadata -> 'accessRowsSnapshot') = 'array'
            then b.metadata -> 'accessRowsSnapshot' else '[]'::jsonb end
        ) row_data
        where coalesce(row_data ->> 'date', row_data ->> 'class_date', '') = d.class_date::text
      ) snapshot
      where b.source = 'access-daily'
        and b.status = 'completed'
        and (
          coalesce(b.metadata -> 'sourceDates', '[]'::jsonb) ? d.class_date::text
    ...
    select class_date, id, expected_rows, actual_rows,
      id is not null and (actual_rows = 0 or (expected_rows > 0 and actual_rows < expected_rows)) as incomplete
    from latest
    ...
    where a.class_date >= v_start
      and a.class_date < v_end
      and a.teacher_name = v_teacher
      and (b.id is null or b.incomplete or a.import_batch_id = b.id)
  )
  select jsonb_build_object(
    ...
    'incompleteDates', coalesce((
      select jsonb_agg(class_date order by class_date)
      from batch_state where incomplete
    ), '[]'::jsonb),
    'attendanceRows', coalesce((
      select jsonb_agg(to_jsonb(a) order by a.class_date, a.start_time_text, a.student_name)
      from filtered_attendance a
    ), '[]'::jsonb),
    'classLogRows', coalesce((
      select jsonb_agg(to_jsonb(l) order by l.class_date, l.student_name)
      from public.class_log_rows l
      where l.class_date >= v_start and l.class_date < v_end and l.teacher_name = v_teacher
    ), '[]'::jsonb),
    'signatureRows', coalesce((
      select jsonb_agg(to_jsonb(s) order by s.class_date)
      from public.signatures s
      where s.class_date >= v_start and s.class_date < v_end and s.teacher_name = v_teacher
    ), '[]'::jsonb)
  ) into v_result;
...
comment on function public.portal_get_teacher_hours_live(jsonb) is
  'Returns latest complete Access batch rows plus class-log/signature agreement evidence for the caller-scoped teacher.';

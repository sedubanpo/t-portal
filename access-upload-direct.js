(function(global) {
  ...
  function attendanceKey(row) {
    return [row.class_date,normName(row.teacher_name),studentKey(row.student_name||row.raw_student),timeKey(row.start_time_text),timeKey(row.end_time_text),categoryKey(row.category),normText(row.campus)].join('|');
  }
  function dedupe(rows, rejectConflicts = false) {
    const map = new Map();
    (rows || []).forEach(row => {
      const key = attendanceKey(row), prev = map.get(key);
      if (prev && rejectConflicts && diff(prev, row).length) {
        throw new Error(`${row.class_date} ${row.student_name} · ${row.teacher_name}: 같은 수업의 출결·시간·비고가 서로 다른 중복 행이 있습니다. 원본에서 한 행으로 정리한 뒤 다시 검증해 주세요.`);
      }
      if (!prev || String(row.updated_at || '') > String(prev.updated_at || '')) map.set(key, row);
    });
    return [...map.values()];
  }
